...
from app.core.pp_onnx.pp_lcnet_doc_onnx_new import PPLCNetDocONNX
...
def main():
    ...
    try:
        cls_result = cls_model.classify(image)
        print(f"Document orientation: {cls_result['angle']}° (conf: {cls_result['confidence']:.2f})")
        
        # Rotate image based on detected angle
        angle = int(cls_result['angle'])
        if angle == 90:
            rotated_image = cv2.rotate(image, cv2.ROTATE_90_COUNTERCLOCKWISE)  # Correct direction
        elif angle == 180:
            rotated_image = cv2.rotate(image, cv2.ROTATE_180)
        elif angle == 270:
            rotated_image = cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE)  # Correct direction
        else:
            rotated_image = image.copy()
        
        print(f"Image rotated by {angle}° for processing")
    except Exception as e:
    ...
    try:
        detections = det_model.detect(rotated_image, conf_threshold=0.3, use_close=True)
